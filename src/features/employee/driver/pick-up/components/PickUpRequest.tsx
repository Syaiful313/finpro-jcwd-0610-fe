...
export default function PickUpRequestList() {
  ...
  const PickUpRequestCard = ({ request }: { request: any }) => (
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                <p className="text-muted-foreground text-sm">
                  Showing {filteredRequests.length} of {totalElements} Pick up
                  requests
                </p>
                {totalPages > 0 && (
                  <p className="text-muted-foreground text-sm">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {filteredRequests.map((request) => (
                <PickUpRequestCard key={request.orderId} request={request} />
              ))}
            </div>
          ) : (

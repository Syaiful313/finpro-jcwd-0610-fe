...
const EmployeeCard = ({
  ...
  const getRoleColors = (role: string) => {
    const roleColors = {
      WORKER: { avatar: "bg-gradient-to-br from-blue-500 to-blue-600" },
      DRIVER: { avatar: "bg-gradient-to-br from-purple-500 to-purple-600" },
    };
      ...
      <div className="p-3.5">
        ...
        <div className="mb-3 space-y-2">
          <div className="flex justify-between text-sm">
            <span className="font-medium text-gray-700">Total Pekerjaan:</span>
          ...
          {employee.role === "WORKER" ? (
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="rounded-lg bg-blue-50 p-2 text-center">
                ...
                <div className="text-gray-600">Cuci</div>
              </div>
              <div className="rounded-lg bg-green-50 p-2 text-center">
                <div className="font-semibold text-green-600">
                  {(employee as any).totalIroningJobs}
                </div>
                <div className="text-gray-600">Setrika</div>
              </div>
  ...
  <TableRow className="border-b hover:bg-gray-50">
    ...
    <TableCell className={getCellClass("name")}>
      <div className="flex flex-col">
        <div className="font-medium break-words">{employee.employeeName}</div>
    ...
    <TableCell className={getCellClass("jobs")}>
      {employee.role === "WORKER" ? (
        <div className="space-y-1 text-xs">
          <div>Cuci: {(employee as any).totalWashingJobs}</div>
          <div>Setrika: {(employee as any).totalIroningJobs}</div>
          <div>Packing: {(employee as any).totalPackingJobs}</div>
        </div>
      ) : (
        <div className="space-y-1 text-xs">
          <div>Pickup: {(employee as any).totalPickupJobs}</div>
          <div>Delivery: {(employee as any).totalDeliveryJobs}</div>
        </div>
      )}
    </TableCell>
    ...
    <TableCell className={getCellClass("total")}>
      <div className="space-y-1">
        <div className="font-medium">{employee.totalJobs}</div>
  ...
  if (!session) {
    return (
      <div className="flex h-64 items-center justify-center px-1">
        ...
        <span className="ml-2 text-sm sm:text-base">Memuat sesi...</span>
      </div>
    );
  }
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                {isAdmin && (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="flex h-12 flex-1 items-center justify-center gap-2 rounded-xl border-2 border-gray-200 bg-white px-4 text-sm text-gray-700 transition-colors hover:bg-gray-50">
                        <Filter className="h-4 w-4" />
              ...
              <Button
                ...
                className="h-10 w-full rounded-xl border-gray-200 text-sm"
              >
                Reset Filter
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 lg:w-80">
              <div>
                ...
                <Input
                  ...
                  className="rounded-xl border-gray-200 text-sm focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-700">
        ...
        <div className="block sm:hidden">
          {isLoading ? (
            <div className="flex h-32 items-center justify-center">
              ...
              <span className="text-sm">Memuat data performa...</span>
            </div>
          ) : error ? (
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b">
                  ...
                  <TableHead className="hidden w-32 text-center text-xs sm:w-40 sm:text-sm lg:table-cell">
                    Detail Pekerjaan
                  </TableHead>
